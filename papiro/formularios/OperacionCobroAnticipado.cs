...
{
    public partial class OperacionCobroAnticipado : Form
    {
        ...
        public OperacionCobroAnticipado(usuarios user)
        {
            InitializeComponent();
            _user = user;
        ...
        public OperacionCobroAnticipado(usuarios user, operaciones operacionPendiente)
        {
            InitializeComponent();
            _user = user;
        ...
        private void OperacionCobroAnticipado_Load(object sender, EventArgs e)
        {
            ...
            {
                _clientesId.Add(ca.id_cliente);
                cb_cliente.Items.Add(ca.cliente.nombre);
            }
            cb_cliente.SelectedIndex = 0;
        }
        ...
        private void cb_cliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var ca = _entities.cobro_anticipado.Where(c => c.id_cliente == id).Single();
            cantidadLabel.Text = Math.Round(ca.cantidad, 2).ToString(CultureInfo.InvariantCulture);
        ...
        private void cb_tipo_operacion_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void operaciones3erosdataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void operaciones3erosdataGridView_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
        ...
        private void operaciones3erosdataGridView_RowsRemoved(object sender, DataGridViewRowsRemovedEventArgs e)
        {
            _proveedorIds.RemoveAt(e.RowIndex);
            UpdateOperaciones3ErosTotal();
        }

        private void operaciones3erosdataGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
        ...
        private void operaciones3erosdataGridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void rBEstatal_CheckedChanged_1(object sender, EventArgs e)
        {
        ...
        private void rBParticular_CheckedChanged_1(object sender, EventArgs e)
        {
            cb_cliente.Items.Clear();
            _clientesId.Clear();
            cb_cliente.Items.Add("<Seleccione>");
        ...
        private void S_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show(
                        "Debe ingresar datos válidos para realizar la operación, el monto total no puede ser 0",
                        "Error en la entrada de datos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                if(cb_cliente.SelectedIndex == 0 && _clientesId[cb_cliente.SelectedIndex] == 0)
                {
                    MessageBox.Show("Debe seleccionar un cliente para poder realizar la operación",
                                    "Error en la entrada de datos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                if (montTotal != 0)
                {
                    ...
                    //compruebo que el cobro anticipado tiene dinero para cargar la operacion
                    int id = _clientesId[cb_cliente.SelectedIndex];
                    var ca = _entities.cobro_anticipado.Where(c => c.id_cliente == id).Single();

                    if(ca.cantidad < montTotal)
                    {
                        MessageBox.Show(
                            "El cliente no puede realizar la operación, pues el costo de la operación\nsupera la cantidad que le queda del pago anticipado",
                            "Capital insuficiente", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    ca.cantidad -= montTotal;
                    //cobro anticipado
                    _entities.AddTosubmayor_cobro_anticipado(new submayor_cobro_anticipado
                    {
                        ...
                        descripcion = "Se realiza operación de impresión.",
                        debito = montTotal,
                        saldo = balance.cobro_anticipado - montTotal
                    });
                    balance.cobro_anticipado -= montTotal;
                    if (ca.cantidad == 0)
                    {
                        MessageBox.Show(
                            "EL capital del pago anticipado ha sido completamente gastado",
                            "Capital insuficiente", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        _entities.AddTobitacora(new bitacora
                        {
                            id_usuario = this._user.id,
                            accion_realizada = "Se ha eliminado el cobro anticipado al cliente " + ca.cliente.nombre,
                            fecha = DateTime.Now,
                            nombre_usuario = this._user.nombre
                        });
                        _entities.DeleteObject(ca);
