...
{
    public partial class GestionarTipoOperacion : Form
    {
        ...
        public GestionarTipoOperacion(usuarios user)
        {
            InitializeComponent();
            _user = user;
        ...
        private void ListarTiposOperacion()
        {
            ...
            foreach (var tipoOperacion in _entities.tipo_operacion)
            {
        ...
        private void ResetEditForm(bool resetText = false)
        {
            ...
            topOpGroupBox.Text = @"Detalles de tipo de operación";
        ...
        private void ModificarButtonClick(object sender, EventArgs e)
        {
            switch (_estado)
            {
                case Estado.List:
                    ...
                    topOpGroupBox.Text = @"Edición de tipo de operación";
        ...
        private string ValidarDatosForm(int idTipOp)
        {
            ...
            if (_entities.tipo_operacion.Any(tip => tip.valor == descripcionTextBox.Text.Trim() && tip.id != idTipOp))
        ...
        private void Agregar()
        {
            try
            {
                ...
                {
                    MessageBox.Show(validationMessage, @"Agregar tipo de operación",
                                    MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                
                var tipOp = new tipo_operacion
                             {
                                 valor = descripcionTextBox.Text.Trim(),
                                 valor_operacion = valorNumericUpDown.Value
                             };

                _entities.AddTotipo_operacion(tipOp);
                ...
                var bit = new bitacora
                              {
                                  ...
                                  fecha = DateTime.Now,
                                  accion_realizada =
                                      string.Format("Se agregó el tipo de operación: {0}", tipOp.valor)
                              };
        ...
        private void Actualizar(int selectedIndex)
        {
            try
            {
                ...
                if (validationMessage != "")
                {
                    MessageBox.Show(validationMessage, @"Modificar tipo de operación",
                                    MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                ...
                var tipoOperacion =
                    (tipo_operacion)
                    _entities.GetObjectByKey(new EntityKey("papiro_finalEntities.tipo_operacion", "id", idTipOp));
                ...
                var bit = new bitacora
                              {
                                  ...
                                  fecha = DateTime.Now,
                                  accion_realizada =
                                      string.Format("Se actualizó la tipo de operación: {0}", tipoOperacion.valor)
                              };
        ...
        private void NuevoButtonClick(object sender, EventArgs e)
        {
            switch (_estado)
            {
                case Estado.List:
                    ...
                    topOpGroupBox.Text = @"Nuevo tipo de operación";
        ...
        private void Eliminar()
        {
            ...
            if (countChecked == 0)
            {
                MessageBox.Show(@"Debe seleccionar al menos un tipo de operación.", @"Eliminar tipo de operación",
                                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            if (countChecked == 1)
            {
                if (MessageBox.Show(@"¿Estás seguro de querer eliminar el tipo de operación?",
                                    @"Eliminar tipo de operación", MessageBoxButtons.YesNo, MessageBoxIcon.Information)
                    == DialogResult.No)
                {
                    return;
                }
            ...
            try
            {
                ...
                foreach (tipo_operacion tipOpByKey in
                                from ListViewItem checkedItem in checkedListViewItemCollection
                                select _entities.GetObjectByKey(new EntityKey("papiro_finalEntities.tipo_operacion",
                                                                "id", _tipOpIds[checkedItem.Index])))
                {
                    if (_entities.operaciones.Any(op => op.id_tipo_operacion == tipOpByKey.id))
                    {
                    ...
                    var b = new bitacora
                                {
                                    ...
                                    nombre_usuario = _user.login_nombre,
                                    fecha = DateTime.Now,
                                    accion_realizada = string.Format("Eliminado el tipo de operación: {0}", tipOpByKey.valor)
                                };
                ...
                if (tipOpNoElimino != "")
                {
                    ...
                    if (tipOpNoElimino.IndexOf(',') == -1)
                        MessageBox.Show(
                            @"No se pudo eliminar el tipo de operación: '" + tipOpNoElimino +
                            @"', porque tiene operaciones asociadas.", @"Eliminar tipo de operación",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    else
                        MessageBox.Show(
                            @"No se pudieron eliminar los tipos de operación: (" + tipOpNoElimino +
                            @"), porque tienen operaciones asociadas.", @"Eliminar tipo de operación",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
