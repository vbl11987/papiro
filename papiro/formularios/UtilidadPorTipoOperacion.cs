...
{
    public partial class UtilidadPorTipoOperacion : Form
    {
        private int id_top;
        private DateTime desde;
        private DateTime hasta;
        public UtilidadPorTipoOperacion(int id_top, DateTime desde, DateTime hasta)
        {
            InitializeComponent();
            this.id_top = id_top;
            this.desde = desde;
        ...
        private void UtilidadPorTipoOperacion_Load(object sender, EventArgs e)
        {
                            ...
                            new ListViewItem(new[]
                                    {
                                        ...
                                        op.tipo_operacion.valor,
                                         Math.Round((op.monto - op.costo - op.gasto.Value), 2).ToString(CultureInfo.InvariantCulture),
                                         op.fecha.ToString("dd/MM/yyyy")
                                    }
                                )
                            );
                            total += op.monto - op.costo - op.gasto.Value;
                        }
                    }
                    printableLV.Items.Add(
                            new ListViewItem(new[]
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (filtroDesde.Value.Date == filtroHasta.Value.Date)
                this.printableLV.Title = "Utilidad por tipo de operación";
            else
            {
                this.printableLV.Title = "Utilidad por tipo de operación desde el " + filtroDesde.Value.Date.ToString("dd/MM/yyyy") + " hasta el " + filtroHasta.Value.Date.ToString("dd/MM/yyyy");
            }
