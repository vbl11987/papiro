...
{
    public partial class EstadoResultadosVerDetallesUtilidades : Form
    {
        private DateTime desde;
        private DateTime hasta;
        private readonly List<int> _tipoOperacionId;
        public EstadoResultadosVerDetallesUtilidades(DateTime desde, DateTime hasta)
        {
            InitializeComponent();
        ...
        private void EstadoResultadosVerDetallesUtilidades_Load(object sender, EventArgs e)
        {
        ...
        public void ActualizarReporte()
        {
                    ...
                    foreach (var t_op in entities.tipo_operacion)
                    {
                        ...
                        foreach (var op in entities.operaciones)
                        {
                            ...
                            if (op.tipo_operacion.id == t_op.id)
                                operacion_utilidad += (op.monto - op.costo - op.gasto.Value);
                        }
                        printableLV.Items.Add(
                            new ListViewItem(new[]
                                    {
                                        t_op.valor,
                                         Math.Round(operacion_utilidad, 2).ToString(CultureInfo.InvariantCulture)
                                    }
                                )
                            );
                        total += operacion_utilidad;
                    }
                    printableLV.Items.Add(
                           new ListViewItem(new[]
                                    {
            ...
            catch (Exception exception)
            {
                MessageBox.Show(
                    ...
                    @"Detalles de utilidades", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                this.printableLV.Title = "Detalles de utilidades";
            }
            else
            {
                this.printableLV.Title = "Detalles de utilidades desde el " + filtroDesde.Value.Date.ToString("dd/MM/yyyy") + " hasta el " + filtroHasta.Value.Date.ToString("dd/MM/yyyy");
            }
        ...
        private void verdetalles_Click(object sender, EventArgs e)
        {
            ...
            UtilidadPorTipoOperacion u = new UtilidadPorTipoOperacion(_tipoOperacionId[cbxTipoOP.SelectedIndex], this.desde, this.hasta);
            u.ShowDialog();
        }
